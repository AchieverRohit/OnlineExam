...
public class ApplicationUserService : IApplicationUserService
{
    ...
    /// <exception cref="ArgumentNullException">Thrown when the type parameter is null or empty.</exception>
    public async Task<ListResponse<string>> GetTypes(string type, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var roles = await _dbContext.Lookups
                .Where(role => role.Identifier.Equals(type))
                .Select(role => role.Value)
                .ToListAsync(cancellationToken);

            if (roles.Any())
            {
                return new ListResponse<string> { Data = roles, Status = HttpStatusCode.OK };
            }

            return new ListResponse<string>
            {
                Status = HttpStatusCode.BadRequest,
                Messages = new List<ResponseMessage> { new ResponseMessage { Message = "Invalid type parameter." } }
            };
        }
        catch (Exception ex)
        {
            // Log the exception (log mechanism not shown here)
            return new ListResponse<string>
            {
